...
# ============================================================
BOT_TOKEN = "8686421047:AAEwOAWjK5x5zNDd5BUDBV4fTV68CtQPfgM"  # Set your BotFather token here; no environment variable is used.
ADMIN_ID = 8239453740
...
def register_user(user):
    con = db()
    try:
        existing = con.execute("SELECT 1 FROM users WHERE id=?", (user.id,)).fetchone()
...
async def notify_new_user(bot, user):
    """Notify every configured admin when a user joins for the first time."""
    username = f"@{esc(user.username)}" if user.username else "—"
    name = esc(user.full_name or user.first_name or "Unknown")
    text = (
        "🆕 <b>NEW USER JOINED</b>\n\n"
        f"👤 <b>Name:</b> {name}\n"
        f"🔗 <b>Username:</b> {username}\n"
        f"🆔 <b>User ID:</b> <code>{user.id}</code>\n"
        f"📅 <b>Joined:</b> {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M UTC')}"
    )
    for admin_id in current_admin_ids():
        try:
            await bot.send_message(admin_id, text, parse_mode=ParseMode.HTML)
        except TelegramError as exc:
            log_event("New user notification failed", f"{admin_id}: {exc}")
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_new:
        await notify_new_user(context.bot, update.effective_user)
